...
from deepStrain.utils import seqParse
from deepStrain.utils import pathoUtilsA
from deepStrain.pathodb import dbUtils
from deepStrain.pathoreport import xmlReport
...
def processPathoReport(pathoReportOptions):
    ...
    if pathoReportOptions.samFile is not None:
        ...
        (_, _, _, _, _, _, _, _, _, _, _) = write_tsv_report(outTsv, nR, nG,
                                                             pi, genomes, initPi, bestHitInitial, bestHitInitialReads,
                                                             bestHitFinal,
                                                             bestHitFinalReads, level1Initial, level2Initial,
                                                             level1Final, level2Final, header,
            ...
            if entr:
                ...
                for j in entr:  # select which subgi sits within the covered genomic regions
                    ...
                    for i in range(C):
                        ...
                        notCoveredA -= pathoUtilsA.segments_intersect(aStbp, aEdbp, covRange[i][START],
                                                                      covRange[i][END])
