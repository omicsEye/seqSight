...
from deepStrain.utils import pathoUtilsA
from deepStrain.utils import samUtils
from deepStrain.pathoreport import PathoReportA
...
def pathoscope_reassign(pathoIdOptions):
    ...
    if float(os.stat(ali_file).st_size) < 1.0:
        print
        'the alignment file [%s] is empty.' % ali_file
        sys.exit(1)
    ...
    if ali_format == 'gnu-sam':
        ...
        if verbose:
            print
            "parsing gnu-sam file/likelihood score/reads and mapped genomes..."
    elif ali_format == 'sam':  # standard sam
        aliFormat = 1
        if verbose:
            print
            "parsing sam file/likelihood score/reads and mapped genomes..."
    elif ali_format == 'bl8':  # blat m8 format
        aliFormat = 2
        if verbose:
            print
            "parsing bl8 file/likelihood score/reads and mapped genomes..."
    else:
        print
        "unknown alignment format file..."
        return
