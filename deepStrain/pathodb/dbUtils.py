...
def mysql_update_anno_gi2(con, gi, ref_name, seq_len, taxon, product, has_sub):
    with con:
        cur = con.cursor()
        mysql_ins_cmd = "insert into giAnnoT(gi,ref_name,seq_len,taxon,product,has_sub) values(%d,\'%s\',%d,%d,\'%s\',%d)" % (
...
def mysql_update_ti(con, ti, organism, lineage):
    ...
    with con:
        ...
        if entries:  # there is a prev record and note that this case will correspond to the case only single gi
            ...
            mysql_ins_cmd = "insert into cj_taxonT(taxon,organism,lineage) values(%d,\'%s\',\'%s\')" % (
            ti, organism, lineage)
            # print mysql_ins_cmd #debug
            cur.execute(mysql_ins_cmd)
    return ti
...
def mysql_update_delim(con, gi, sub_gi, gene, locus_tag, protein_id, strand, stbp, edbp):
    ...
    with con:
        ...
        mysql_ins_cmd = "insert into giDelimT(gi,sub_gi,gene,locus_tag,protein_id,strand,stBp,edBp) values(%d,%d,\'%s\',\'%s\',\'%s\',\'%s\',%d,%d)" % (
...
def init_mysql_innocentive(MySqlConf, reset_table):
    ...
    try:
        con = mdb.connect(host=MySqlConf[HOST_NAME], port=MySqlConf[MYSQL_PORT], user=MySqlConf[USER],
                          passwd=MySqlConf[PASSWORD], db=MySqlConf[DEFAULT_DB])
    ...
    with con:
        ...
        cur.execute(
            "CREATE TABLE giDelimT (id INT NOT NULL AUTO_INCREMENT PRIMARY KEY, gi INT NOT NULL, sub_gi INT, gene VARCHAR(64), locus_tag VARCHAR(64), protein_id VARCHAR(64), strand CHAR(1), stbp INT, edbp INT)")  # , INDEX idxGis(gi,sub_gi)

        # ------------------
        # create gi annotation table
        cur.execute("drop table if exists giAnnoT")
        cur.execute(
            "create table giAnnoT (id INT NOT NULL AUTO_INCREMENT PRIMARY KEY, gi INT NOT NULL, ref_name VARCHAR(64), seq_len INT, taxon INT, product VARCHAR(1024), has_sub INT)")  # , INDEX idxGi (gi)
        # -------------------
        # create taxon_table

        cur.execute("drop table if exists cj_taxonT")
        cur.execute(
            "create table cj_taxonT (id INT NOT NULL AUTO_INCREMENT PRIMARY KEY, taxon INT NOT NULL, dbSize BIGINT UNSIGNED, organism VARCHAR(128), lineage VARCHAR(2048))")  # , INDEX idxTaxon (taxon)
...
def phylo_node2mysql(con, nodesDfn):
    with con:
        ...
        for i in fp:
            j = i.split('\t|\t')
            mysql_ins_cmd = "insert into phylo_nodesT(tax_id,parent_tax_id,rank,embl_code,division_id,inherited_div_flag,genetic_code_id,inherited_GC_flag,mitochondrial_genetic_code_id,inherited_MGC_flag,GenBank_hidden_flag,hidden_subtree_root_flag,comments) values (%d,%d,\'%s\',\'%s\',%d,%d,%d,%d,%d,%d,%d,%d,\'%s\')" % (
            int(j[0]), int(j[1]), j[2], j[3], int(j[4]), int(j[5]), int(j[6]), int(j[7]), int(j[8]), int(j[9]),
            int(j[10]), int(j[11]), j[12])
            cur.execute(mysql_ins_cmd)
        fp.close()
...
def taxon_rel_in_hash_mysql(con, workD):
    ...
    with con:
        ...
        if entr:
            ...
            print
            'check your taxonomy table in pathodb database!'
...
def mysql_update_anno_sub_gi(con, gi, ref_name, taxon, product, gene, protein_id):
    ...
    with con:
        ...
        if entries:
            ...
            mysql_ins_cmd = "insert into giAnnoT(gi,ref_name,taxon,product,gene,protein_id) values(%d,\'%s\',%d,\'%s\',\'%s\',\'%s\')" % (
